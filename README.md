...
Я сделала 3D проект в Unity. На сцену я добавила пустой GameObject, к которому присоединила перцептрон.
...
В ходе выполнения работы в консоль выводилось количество ошибок на каждой эпохе обучения. Я разместила их в таблицу EXCEL и на основе этих данных построила графики:

![image](https://user-images.githubusercontent.com/113617617/202650523-8fad9b7f-1113-4a79-86dc-da335bd51619.png)

Как видно из графиков, в среднем перцептрону хватает трех эпох. Во время расчёта AND перцептрону понадобилось аж 6 эпох, вероятность такого весьма мала.
...
Преимущество компьтера перед человеком в том, что для компьютера это всего лишь количество значений.

## Задание 3
### Построить визуальную модель работы перцептрона на цсене Unity.
Для выполнения задания я добавила на сцену 4 пару кубов с материалами mat0 и mat1.
...
## Выводы
 В ходе выполнения работы я применила перцептрон, вникла в его код. Построила графики зависимости качества обучения перцептрона от количества эпох на простейших логических функциях. Так же я построила модель, визуализирующую работы логических операторов (обученного перцептрона).
